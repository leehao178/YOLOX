...
class BaseHeadLessExp(MyExp):
    def __init__(self):
        super(BaseHeadLessExp, self).__init__()
        # ---------------- model config ---------------- #
        self.num_classes = 15
        self.depth = 0.67
        self.width = 0.75
        ...
        self.data_dir = "/home/danny/DataSet/car_head_coco"
    ...
    def get_model(self):
        from yolox.models import YOLOXHeadLess, YOLOPAFPN, YOLOXRotateHeadLessHead
        ...
        if getattr(self, "model", None) is None:
            ...
            head = YOLOXRotateHeadLessHead(num_classes=self.num_classes, 
    ...
    def get_data_loader(
        ...
        from yolox.data import (
            ...
            DataLoader,
            InfiniteSampler,
            MosaicHeadLessDetection,
            TrainTransformHeadLess,
        ...
        with wait_for_the_master(local_rank):
            dataset = COCOHeadLessDataset(
                data_dir=self.data_dir,
                json_file=self.train_ann,
                name="train2017",
                img_size=self.input_size,
                preproc=TrainTransformHeadLess(
                    max_labels=9999,
                    flip_prob=0.0,
        ...
        dataset = MosaicHeadLessDetection(
            dataset,
            mosaic=not no_aug,
            img_size=self.input_size,
            preproc=TrainTransformHeadLess(
    ...
    def get_eval_loader(self, batch_size, is_distributed, testdev=False, legacy=False):
        from yolox.data import COCOHeadLessDataset, ValTransformHeadLess

        valdataset = COCOHeadLessDataset(
            data_dir=self.data_dir,
            json_file=self.val_ann if not testdev else "instances_dev2017.json",
            name="val2017" if not testdev else "dev2017",
            img_size=self.test_size,
            preproc=ValTransformHeadLess(legacy=legacy),
        )
    ...
    def get_evaluator(self, batch_size, is_distributed, testdev=False, legacy=False):
        from yolox.evaluators import Dota10HeadLessEvaluator

        val_loader = self.get_eval_loader(batch_size, is_distributed, testdev, legacy)
        evaluator = Dota10HeadLessEvaluator(
